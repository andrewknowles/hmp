...
// get customer code
		$customer=trim($row ['Customer_No']);
		foreach ($customerlist as $int)
		{
			if (trim($int['old']) == trim($row ['Customer_No']))
			{
				$customer = $int['new'];
				break;
			}
...
// salesman
		$salesman = trim($row['Salesman_Lookup']);
		$salesman = str_replace("'", " ", $salesman);
		$salesman = str_replace("*", "-", $salesman);
		
// equipment serial numbers
/*		$newserial = '00000000000000000';
		if ($row ['Japan_Info_Serial_No'] <> '#N/A') {
			$newserial = $row ['Full_Serial_No'];
		} else { */
...
//		}
		...
		$year = substr($firstused,0,4);
		$firstused = substr($firstused,8,2).'/'.substr($firstused,5,2).'/'.substr($firstused,2,2);


//Purchase Date
		$dp1 = $row ['EM_ACQUIRED_DATE'];		
		$datepurch = substr($dp1,8,2).'/'.substr($dp1,5,2).'/'.substr($dp1,2,2);
		
//Sold Date
		$sd1 = $row ['EM_SOLD_DATE'];
		$datesold = substr($sd1,8,2).'/'.substr($sd1,5,2).'/'.substr($sd1,2,2);
