...
//Load manufacturer table ATTENTION this is table manufacturerm for equipment manufacturers
//this table can map several old manufacturers to 1 new manufacturer therefore the old column must be unique
$modmfrlist = $Mydb->load_values('manufacturerm');
...
// get equipment model
		...
		{
			if (trim($int['old']) == trim($row ['EM3_MODEL']))
			{
				$model = $int['new'];
				$group = $int['groupe'];
				$series = $int['series'];
...
// get customer code
		...
		{
			if (trim($int['old']) == trim($row ['NA_CUSTOMER']))
			{
				$customer = $int['new'];
				break;
			}
...
// concatenate old customer number and name to place in the notes field
		$custname = trim($row['NA_NAME']);
		$custname = str_replace("'", "", $custname);
		$custname = str_replace("*", "-", $custname);
//		$custname = ucwords(strtolower($custname));
		$custnumbername = $row ['NA_CUSTOMER'].' -- '.$custname;
...
// equipment serial numbers
		$newserial = '00000000000000000';
		if ($row ['Japan_Info_Serial_No1'] <> '#N/A') {
			$newserial = $row ['Japan_Info_Serial_No1'];
		} else {
			$newserial = $row ['EM3_SERIAL'];
		}
		
//First used date
		$firstused = '01/01/2000';
		
		if (strlen($row ['MW_START_DATE_1']) <> 0)
		{
			$firstused = $row ['MW_START_DATE_1'];
		} else {
			$firstused = $row ['EM_SOLD_DATE'];
		}

		$year = substr($firstused,6,4);
		$firstused = substr($firstused,0,2).'/'.substr($firstused,3,2).'/'.substr($firstused, 6,4);


//Purchase Date
		$dp1 = $row ['EM_ACQUIRED_DATE'];		
		$datepurch = substr($dp1,0,2).'/'.substr($dp1,3,2).'/'.substr($dp1,6,4);
		
//Sold Date
		$sd1 = $row ['EM_SOLD_DATE'];
		$datesold = substr($sd1,0,2).'/'.substr($sd1,3,2).'/'.substr($sd1,6,4);
		
//Engine No

		if ($row ['Japan_Info_Engine_No'] == '#N/A')
		{
		$engno = $row ['Engine_No'];
		} else {
			$engno = $row ['Japan_Info_Engine_No'];
		}
...
//		echo 'Eng '.$row ['Japan_Info_Engine_No'];
		$rows ++;
		...
		$class = '500';
			...
			case '2':
					$class = '520';
					$subclass = '121';
							$value = $row ['EM_FREIGHT'];
			break;
			case '3':
			$class = '560';
			$subclass = '120';
			$value = $row ['EM_REPAIRS_LTD'];
					break;
					case '4':
					$class = '160';
							$subclass = '110';
					$value = $row ['EM_OVER_TRADE'];
							break;
							case '5':
								$class = '100';
								$subclass = '100';
